...
def compute_losses(batch, res, criterion_dict, args):
    ...
    #Additional variables for the anchors: 
    ...
    if args.predict_lang_anchors:
        if 'nr' in args.referit3D_file:
            lang_out = max_num_anchors + 1
        else:
            lang_out = 2 
        n_obj_classes = args.n_obj_classes + 1  # +1 to include the no_obj class
    else:
        lang_out = 1
    ...
    if args.anchors != 'none':
        ...
        if args.s_vs_n_weight is not None:
            ...
            total_loss = criterion(LOGITS_reshaped, trg_pass_reshaped)
        """
        total_loss = 0
        for i in range(trg_pass.shape[-1]):
            #total_loss += F.cross_entropy(logits[:, i], trg_pass[:, i])
            total_loss += criterion(logits[:, i], trg_pass[:, i])
        """
        
        # Aux_logit loss
        if res['aux_logits'] != None:
            AUX_LOGITS = res['aux_logits']
            total_loss += criterion_dict['logit_aux'](AUX_LOGITS.reshape(-1, AUX_LOGITS.shape[2]), trg_pass_reshaped)
            """
            for i in range(trg_pass.shape[-1]):
                #total_loss += F.cross_entropy(AUX_LOGITS[:, i], trg_pass[:, i])
                total_loss += criterion_dict['logit_aux'](AUX_LOGITS[:, i], trg_pass[:, i])
            """
    ...
    if args.context_2d=='unaligned':
        ...
        if args.s_vs_n_weight is not None:
            ...
            if args.anchors != 'none':
                trg_pass = torch.cat((batch['anchors_pos'], batch['target_pos'].unsqueeze(-1)), -1)  # [N, trg_seq_length]
                trg_pass_reshaped = trg_pass.reshape(-1)  # [N*trg_seq_length]
                LOGITS_2d_reshaped = res['logits_2D'].reshape(-1, res['logits_2D'].shape[2])  # [N*trg_seq_length, num_cls]
                vg2d_loss = criterion(LOGITS_2d_reshaped, trg_pass_reshaped)
            else:
    ...
    if args.lang_cls_alpha > 0:
        ...
        if args.predict_lang_anchors:
            LANG_LOGITS = LANG_LOGITS.contiguous().view(-1, n_obj_classes, lang_out).permute(0, 2, 1).reshape(-1, n_obj_classes) 
            anchor_classes = batch['anchor_classes'].double()
            target_class = batch['target_class'].double()
            lang_tgt = torch.cat((anchor_classes.to(torch.device('cuda')), target_class.unsqueeze(-1)), dim=-1).reshape(-1).long()
            lang_clf_loss = criterion(LANG_LOGITS, lang_tgt)
            
            if res['aux_lang_logits'] is not None:
                res['aux_lang_logits'] = res['aux_lang_logits'].contiguous().view(-1, n_obj_classes, lang_out).permute(0, 2, 1).reshape(-1, n_obj_classes)  # [N*trg_seq_length, num_cls]
                lang_clf_loss += criterion_dict['lang_logit_aux'](LANG_LOGITS, lang_tgt)  
        else:
            lang_clf_loss = criterion(res['lang_logits'], batch['target_class'])
        
        if args.predict_lang_anchors:
            res['lang_logits'] = res['lang_logits'].contiguous().view(-1, n_obj_classes, lang_out)[:,:,-1]
    ...
    if args.anchors != 'none':
        res['logits'] = res['logits'][:, -1]
        res['logits_2D'] = res['logits_2D'][:, -1]
...
def evaluate_on_dataset(model, data_loader, criteria, device, pad_idx, args, randomize=False):
    ...
    for batch in data_loader:
        ...
        # print("TEST lang_clf_loss: ", all_losses['lang_clf_loss'])
