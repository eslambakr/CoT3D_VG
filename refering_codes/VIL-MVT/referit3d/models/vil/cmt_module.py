    ...
    def __init__(self, config):
        ...
        if self.config.spatial_dec:
            ...
            kwargs = {
                ...
                'spatial_multihead': self.config.spatial_multihead,
                'spatial_attn_fusion': self.config.spatial_attn_fusion,
            }
        else:
        ...
        self.layers = _get_clones(decoder_layer, self.config.num_layers)

        loc_layer = nn.Sequential(
            nn.Linear(self.config.dim_loc, self.config.hidden_size),
            nn.LayerNorm(self.config.hidden_size),
        )
        if self.config.obj_loc_encoding in ['same_0', 'same_all']:
            num_loc_layers = 1
        elif self.config.obj_loc_encoding == 'diff_all':
            num_loc_layers = self.config.num_layers
