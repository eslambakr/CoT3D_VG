...
def train():
    ...
    global_num_iter = 0
    for num_batch in tqdm(range(args.num_train_epochs)):
        ...
        # Save and Eval every epoch:
        """
        checkpoint_dir = mkdir_if_not_exists(
            Path(args.output_dir) / 'checkpoint-{:06d}'.format(num_batch))
        ...
        run_evaluation(args, model, test_dl, device, num_batch)
